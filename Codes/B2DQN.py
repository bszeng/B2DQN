...
class Replay_Buffer():
    ...
    def sample_blr(self, batch_size):
        batch_size_blr = np.ceil(batch_size/num_action).astype('int')
        memory_blr_return = []
        for i in range(len(self.memory)):
            self.memory_blr[self.memory[i].action].append(self.memory[i]) 
        for j in range(num_action):
            memory_blr_return.append(random.sample(self.memory_blr[j], batch_size_blr))
        return memory_blr_return
...
def BayesReg(phiphiT,phiY,alpha,batch_size):
    ...
    phiY *= (1-alpha)
    batch_blr = replay_memory.sample_blr(batch_size)
    for j in range(len(batch_blr)):
    ...
    for h in range(opt.K):
        for i in range(num_action):
            ...
            Cov_W[h][i] = sigma * inv
    return phiphiT,phiY,E_W,Cov_W ###### 20211028
...
def sample_W(E_W,U):
    for h in range(opt.K):
        for i in range(num_action):
            ...
            E_W_[h][i] = E_W[h][i] + nd.dot(U[h][i],sam)[:,0]
    return E_W_
...
for seed in seeds:
    ...
    print('--'*18)
    f_n = 'M'+str(M_ULA)+'_b2dqn_' + str(opt.max_episode) + '_seed' + str(seed) + '_k' + str(opt.K) +'_rbz'+str(opt.replay_buffer_size)+'rsz'+str(opt.replay_start_size)+'bs'+str(opt.batch_size)+'tu'+str(opt.Target_update)+'twu'+str(opt.target_W_update)+'tbs'+str(opt.target_batch_size)+ '.txt' 
    f = open(f_n,'w')
    titles = 'episode,reward,timesteps,successful'
    print(titles)
    f.write('\n' + titles)

    command = 'mkdir b2_'+'k'+str(opt.K)+'_'+str(seed) # Creat a direcotry to store models and scores.
    os.system(command) 
    ...
    E_W_target_ = nd.empty((opt.K,num_action,opt.lastlayer), opt.ctx)
    ...
    for  epis_count in range(1, opt.max_episode+1):
        ...
        k = random.randint(0,opt.K-1) # 随机选择1个head，在整个episode中使用
        for timestep_index in range(1, opt.max_frame+1):

            data = nd.array(state.reshape([1,opt.frame_len,opt.state_size]),opt.ctx) # 1*1*8
            ...
            if total_steps > opt.replay_start_size:
                if total_steps % opt.Target_update == 0 :
                    # Synchronize parameters 同步共享网络的参数
                    check_point = total_steps / (opt.Target_update *100)
                    fdqn = './b2_k%d_%d/Boots_target_%s_%d' % (opt.K, seed, env_name,int(check_point))
                    dqn_.save_parameters(fdqn)
                    target_dqn_.load_parameters(fdqn, opt.ctx)
                    print('b2dqn-ddqn parameters replaced, c_t=' + str(c_t))
                    for h in range(opt.K): # 同步所有heads参数
                        fdqn = './b2_k%d_%d/Boots_target_head_%s_%d_%d' % (opt.K, seed, env_name,int(check_point),h)
        ...
        if epis_count % 1000 == 0:
            ...
            print('b2dqn:'+'M'+str(M_ULA)+',seed'+str(seed) + ',' + results+','+str(local_time))
        f.write('\n' + results)
    f.close()
