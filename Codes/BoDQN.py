...
for seed in seeds:
    ...
    f_n = 'M'+str(M_ULA)+'_bodqn_egreedy_' + str(opt.max_episode) + '_seed' + str(seed) + '_k' + str(opt.K) +'_rbz'+str(opt.replay_buffer_size)+'rsz'+str(opt.replay_start_size)+'bs'+str(opt.batch_size)+'tu'+str(opt.Target_update)+ '.txt' 
    f = open(f_n,'w')
    titles = 'episode,reward,timesteps,successful,duration'
    print(titles)
    f.write('\n' + titles)

    command = 'mkdir bo_egreedy_'+'k'+str(opt.K)+'_'+str(seed) # Creat a direcotry to store models and scores.
    os.system(command)
    ...
    for  epis_count in range(1, opt.max_episode+1):
        ...
        for timestep_index in range(1, opt.max_frame+1):    
            ...
            if total_steps > opt.replay_start_size:
                if total_steps % opt.Target_update == 0 :
                    ...
                    fdqn = './bo_egreedy_k%d_%d/Boots_target_%s_%d' % (opt.K, seed, env_name,int(check_point))
                    dqn.save_parameters(fdqn)
                    target_dqn.load_parameters(fdqn, opt.ctx)
                    print('bodqn_egreedy parameters replaced')
                    for h in range(opt.K):
                        fdqn = './bo_egreedy_k%d_%d/Boots_target_head_%s_%d_%d' % (opt.K, seed, env_name,int(check_point),h)
        ...
        if epis_count % 1000 == 0:
            ...
            print('bodqn_egreedy:'+'M'+str(M_ULA)+',seed'+str(seed) + ',' + results+','+str(local_time))
        f.write('\n' + results)
    f.close()
