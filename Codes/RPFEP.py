...
## Prior_Net   
P_DQN = gluon.nn.Sequential()
with P_DQN.name_scope():
    #first layer
    P_DQN.add(gluon.nn.Dense(24, activation='relu'))
    #second layer
    P_DQN.add(gluon.nn.Dense(24, activation='relu'))
p_dqn = P_DQN
p_dqn.collect_params().initialize(mx.init.Normal(0.02), ctx=opt.ctx)
p_heads = gluon.nn.Sequential()
for _ in range(opt.K):
    #third layer
    p_heads.add(HEAD())
...
for seed in seeds:
    ...
    f_n = 'M'+str(M_ULA)+'_RPFEP_' + str(opt.max_episode) + '_seed' + str(seed) + '_k' + str(opt.K) +'_rbz'+str(opt.replay_buffer_size)+'rsz'+str(opt.replay_start_size)+'bs'+str(opt.batch_size)+'tu'+str(opt.Target_update)+ '.txt' 
    f = open(f_n,'w')
    titles = 'episode,reward,timesteps,successful,duration'
    print(titles)
    f.write('\n' + titles)

    command = 'mkdir RPFEP_'+'k'+str(opt.K)+'_'+str(seed) # Creat a direcotry to store models and scores.
    os.system(command)
    ...
    for  epis_count in range(1, opt.max_episode+1):
        ...
        for timestep_index in range(1, opt.max_frame+1):    
            ...
            if total_steps > opt.replay_start_size:        
                if total_steps % opt.learning_frequency == 0:                    
                    ...
                    for j in range(opt.batch_size):
                        ...
                        head_update = batch[j].head # 产生经验所选择的head                            
                        ...
                        with autograd.record():
                            ...
                            main_q_value = main_q_value + opt.prior_scale * p_heads[head_update](p_dqn(batch_state[j])).detach()
                            ...
                            target_q_value = target_q_value + opt.prior_scale * p_heads[head_update](p_dqn(batch_state[j])).detach()
            ...
            if total_steps > opt.replay_start_size:
                if total_steps % opt.Target_update == 0 :
                    ...
                    fdqn = './RPFEP_k%d_%d/Boots_target_%s_%d' % (opt.K, seed, env_name,int(check_point))
                    dqn.save_parameters(fdqn)
                    target_dqn.load_parameters(fdqn, opt.ctx)
                    print('RPFEP parameters replaced')
                    for h in range(opt.K):
                        fdqn = './RPFEP_k%d_%d/Boots_target_head_%s_%d_%d' % (opt.K, seed, env_name,int(check_point),h)
        ...
        time_per_timestep =  1000. * (end_time - start_time)/timestep_index # ms
        results = '%d,%d,%d,%d,%.2f' % (epis_count, episode_reward, timestep_index, successful,time_per_timestep)
        if epis_count % 1000 == 0:
            local_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            print('RPFEP:'+'M'+str(M_ULA)+',seed'+str(seed) + ',' + results+','+str(local_time))
        f.write('\n' + results)
    f.close()
