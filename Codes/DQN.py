...
class DQN:
    ...
    def update(self):
        ...
        with autograd.record():
            ...
            target_q_value = reward_batch + (1 - done_batch) * self.gamma * max_next_q_value
    ...
    def replace_parameters(self):
        self.main_network.save_parameters('./dqn_'+str(self.seed)+'/dqn_temp_params')
        self.target_network.load_parameters('./dqn_'+str(self.seed)+'/dqn_temp_params')
        print('dqn parameters replaced')
    
    def save_parameters(self):
        self.target_network.save_parameters('./dqn_'+str(self.seed)+'/dqn_target_network_parameters')
        self.main_network.save_parameters('./dqn_'+str(self.seed)+'/dqn_main_network_parameters')

    def load_parameters(self):
        self.target_network.load_parameters('./dqn_'+str(self.seed)+'/dqn_target_network_parameters')
        self.main_network.load_parameters('./dqn_'+str(self.seed)+'/dqn_main_network_parameters')
...
for seed in seeds:
    ...
    command = 'mkdir dqn_'+str(seed) # Creat a direcotry to store models and scores.
    os.system(command)
    ...
    f_n = 'M'+str(M_ULA)+'_dqn_' + str(agent.max_episode) + '_seed' + str(seed) +'_rbz'+str(agent.replay_buffer_size)+'rsz'+str(agent.replay_start_size)+'bs'+str(agent.batch_size)+'tu'+str(agent.target_update)+ '.txt' 
    ...
    for epis_count in range(1, agent.max_episode+1):
        ...
        if epis_count % 1000 == 0:
            ...
            print('dqn:'+'M'+str(M_ULA)+',seed'+str(seed) + ',' + results+','+str(local_time))
        f.write('\n' + results)
    f.close()
