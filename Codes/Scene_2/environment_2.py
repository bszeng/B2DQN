...
class radio_environment:
    '''    
        Observation: 
            ...
            5   Neighbor BS power  1                            5        40W
            6   Neighbor BS power  2                            5        40W
            7   BF codebook index for Serving                   0        M-1
            8   BF codebook index for Neighbor 1            0        M-1
            9   BF codebook index for Neighbor 2          0        M-1
    ...
    def __init__(self, seed):
        ...
        self.x_bs_3, self.y_bs_3 = self.inter_site_distance/2.0, self.inter_site_distance * math.sin(math.radians(60))
    ...
    def reset(self):
        ...
        self.f_n_bs3 = self.np_random.randint(self.M_ULA)
    ...
    def step(self, action):
        state = self.state
        reward = 0
        x_ue_1, y_ue_1, x_ue_2, y_ue_2, pt_serving, pt_interferer_1, pt_interferer_2, f_n_bs1, f_n_bs2, f_n_bs3 = state
        
        if (action != -1): # optimal
    
            power_command_l = action & 0b000001 # 1 power up, 0 power down
            power_command_b_1 = (action & 0b000010) >> 1
            power_command_b_2 = (action & 0b000100) >> 2 
            bf_selection_l = (action & 0b001000) >> 3 # 1 step up, 0 step down
            bf_selection_b_1 = (action & 0b010000) >> 4
            bf_selection_b_2 = (action & 0b100000) >> 5
            ...
            if (power_command_b_1 == 0):
                pt_interferer_1 *= 10**(-1/10.)
            else:
                pt_interferer_1 *= 10**(1/10.)

            if (power_command_b_2 == 0):
                pt_interferer_2 *= 10**(-1/10.)
            else:
                pt_interferer_2 *= 10**(1/10.)
            ...
            if (bf_selection_b_1 == 1):
                f_n_bs2 = (f_n_bs2 + 1) % self.k_oversample*self.M_ULA
            else:
                f_n_bs2 = (f_n_bs2 - 1) % self.k_oversample*self.M_ULA

            if (bf_selection_b_2 == 1):
                f_n_bs3 = (f_n_bs3 + 1) % self.k_oversample*self.M_ULA
            else:
                f_n_bs3 = (f_n_bs3 - 1) % self.k_oversample*self.M_ULA
        ...
        self.f_n_bs3 = f_n_bs3
                
        received_power_ue1, interference_power_ue1, received_ue1_sinr = self._compute_rf(x_ue_1, y_ue_1, pt_serving, pt_interferer_1, pt_interferer_2, is_ue_no=1)
        received_power_ue2, interference_power_ue2, received_ue2_sinr = self._compute_rf(x_ue_2, y_ue_2, pt_serving, pt_interferer_1, pt_interferer_2, is_ue_no=2)
            
        # keep track of quantities...
        self.received_ue1_sinr_dB = received_ue1_sinr 
        self.received_ue2_sinr_dB = received_ue2_sinr 
        self.serving_transmit_power_dBm = 10*np.log10(pt_serving*1e3)
        self.interfering_transmit_power_dBm = 10*np.log10(pt_interferer_1*1e3) + 10*np.log10(pt_interferer_2*1e3) 

        # Did we find a FEASIBLE NON-DEGENERATE solution?
        done = (pt_serving <= self.max_tx_power) and  (pt_serving >= 0) and \
                        (pt_interferer_1 <= self.max_tx_power_interference) and (pt_interferer_1 >= 0) and \
                        (pt_interferer_2 <= self.max_tx_power_interference) and (pt_interferer_2 >= 0) and \
                        (received_ue1_sinr >= self.min_sinr) and (received_ue2_sinr >= self.min_sinr) and  \
                        (received_ue1_sinr >= self.sinr_target) and (received_ue2_sinr >= self.sinr_target)
              
        abort = (pt_serving > self.max_tx_power) or (pt_interferer_1 > self.max_tx_power_interference) or  (pt_interferer_2 > self.max_tx_power_interference) or\
                        (received_ue1_sinr < self.min_sinr) or (received_ue2_sinr < self.min_sinr) or  \
                        (received_ue1_sinr > 70) or (received_ue2_sinr > 70) 
      
        # the reward
        reward = received_ue1_sinr + received_ue2_sinr

        # Update the state.        
        self.state = (x_ue_1, y_ue_1, x_ue_2, y_ue_2, pt_serving, pt_interferer_1, pt_interferer_2, f_n_bs1, f_n_bs2, f_n_bs3)
    ...
    def _compute_rf(self, x_ue, y_ue, pt_bs1, pt_bs2, pt_bs3, is_ue_no):
        T = 290 # Kelvins
        B = 15000 # Hz
        ...
        x_bs_3, y_bs_3 = self.x_bs_3, self.y_bs_3
        ...
        if is_ue_no == 1:
            ...
            # This computes the channel for user in serving BS from the interfering BS 1.
            h_2 = self._compute_channel(x_ue, y_ue, x_bs=x_bs_2, y_bs=y_bs_2)
            # This computes the channel for user in serving BS from the interfering BS 2.
            h_3 = self._compute_channel(x_ue, y_ue, x_bs=x_bs_3, y_bs=y_bs_3) #**#
            received_power = pt_bs1 * abs(np.dot(h_1.conj(), self.F[:, self.f_n_bs1])) ** 2
            interference_power_1 = pt_bs2 * abs(np.dot(h_2.conj(), self.F[:, self.f_n_bs2])) ** 2
            interference_power_2 = pt_bs3 * abs(np.dot(h_3.conj(), self.F[:, self.f_n_bs3])) ** 2
        ...
        if is_ue_no == 2:
            ...
            interference_power_1 = pt_bs1* abs(np.dot(h_2.conj(), self.F[:, self.f_n_bs1])) ** 2
            interference_power_2 = pt_bs3 * abs(np.dot(h_3.conj(), self.F[:, self.f_n_bs3])) ** 2

        interference_plus_noise_power = interference_power_1 + interference_power_2 + noise_power
        received_sinr = 10*np.log10(received_power / interference_plus_noise_power)
        
        return [received_power, interference_power_1 + interference_power_2, received_sinr]
