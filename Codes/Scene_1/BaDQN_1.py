...
for seed in seeds:
    ...
    print('--'*18)
    f_n = 'M'+str(M_ULA)+'_BaDQN_'+str(opt.scene)+'_' +  str(opt.max_episode) + '_seed' + str(seed) +'_rbz'+str(opt.replay_buffer_size)+'rsz'+str(opt.replay_start_size)+'bs'+str(opt.batch_size)+'tu'+str(opt.Target_update)+'twu'+str(opt.target_W_update)+'tbs'+str(opt.target_batch_size)+ '.txt' 
    f = open(f_n,'w')
    titles = 'episode,reward,timesteps,duration'
    f.write( titles)

    command =  'mkdir BaDQN_'+str(opt.scene)+'_M'+str(M_ULA)+'_s'+str(seed)# Creat a direcotry to store models and scores.
    os.system(command)
    ...
    for  epis_count in range(1, opt.max_episode+1):
        ...
        while True:
            ...
            if total_steps > opt.replay_start_size:
                if total_steps % opt.Target_update == 0 :
                    ...
                    fdqn = './BaDQN_%d_M%d_s%d/Boots_target_%s_%d' % (opt.scene,M_ULA,seed, env_name,int(check_point))
                    ...
                    if c_t == opt.target_W_update:
                        phiphiT_last = phiphiT
                        phiY_last = phiY
                        E_W_last = E_W
                        Cov_W_last = Cov_W
                        Cov_W_decom_last = Cov_W_decom
                        ...
                        while (no_sample>0):  
                            ...
                            for ii in range(num_action):
                                try:
                                    ...
                                    no_action += 1                            
                            if (no_action == num_action):
                                c_t = 0
                                print('posterior w updated')
                                break                        
                        if (c_t !=  0):
                            c_t = 0
                            phiphiT = phiphiT_last
                            phiY = phiY_last
                            E_W = E_W_last
                            Cov_W = Cov_W_last
                            Cov_W_decom = Cov_W_decom_last
                            print('posterior w updated')
        ...
        if epis_count % 1000 == 0:
            ...
            print('Scene_'+str(opt.scene)+': BaDQN,'+'M'+str(M_ULA)+',seed'+str(seed) + ',' + results+','+str(local_time))
        f.write('\n' + results)
    f.close()
