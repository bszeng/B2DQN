...
with DQN.name_scope():
    ...
    DQN.add(gluon.nn.Dense(24, activation='relu'))
    DQN.add(gluon.nn.Dense(env.action_space.n))
dqn = DQN
...
with Target_DQN.name_scope():
    ...
    Target_DQN.add(gluon.nn.Dense(24, activation='relu'))
    Target_DQN.add(gluon.nn.Dense(env.action_space.n))
...
for seed in seeds:
    ...
    f_n = 'M'+str(M_ULA)+'_DQN_'+str(opt.scene)+'_' + str(opt.max_episode) + '_seed' + str(seed) +'_rbz'+str(opt.replay_buffer_size)+'rsz'+str(opt.replay_start_size)+'.txt' 
    f = open(f_n,'w')
    titles = 'episode,reward,timesteps,duration'
    f.write( titles)

    command =  'mkdir DQN_'+str(opt.scene)+'_M'+str(M_ULA)+'_s'+str(seed)# Creat a direcotry to store models and scores.
    os.system(command)
    ...
    for  epis_count in range(1, opt.max_episode+1):
        ...
        while True:
            ...
            if sample < effective_eps:
                ...
                action = int(nd.argmax(dqn(data),axis=1).as_in_context(opt.ctx).asscalar())
            ...
            if total_steps > opt.replay_start_size:        
                if total_steps % opt.learning_frequency == 0:                    
                    ...
                    with autograd.record():
                        # get the Q(s,a)
                        all_current_q_value = dqn(batch_state)
                        main_q_value = nd.pick(all_current_q_value,batch_action,1)                        
                        # get next action from main network
                        all_next_q_value = target_dqn(batch_state_next).detach() # only get gradient of main network
            ...
            if total_steps > opt.replay_start_size:
                if total_steps % opt.Target_update == 0 :
                    ...
                    fdqn = './DQN_%d_M%d_s%d/Boots_target_%s_%d' % (opt.scene,M_ULA,seed, env_name,int(check_point))
        ...
        if epis_count % 1000 == 0:
            ...
            print('Scene_'+str(opt.scene)+': DQN,'+'M'+str(M_ULA)+',seed'+str(seed) + ',' + results+','+str(local_time))
        f.write('\n' + results)
    f.close()
